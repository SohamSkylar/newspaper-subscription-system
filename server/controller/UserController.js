...
const getSpecificUser = async (req, res) => {
  mysqlPool.getConnection((err, connection) => {
    if (err) return res.send({ msg: err.message });
      ...
      connection.query(sqlQuery, req.params.email, (err, result) => {
        if (err) return res.send({ msg: "Invalid Email" });
        else if (result) {
          if (result.length === 0) {
...
const getSpecificUserID = async (req, res) => {
  mysqlPool.getConnection((err, connection) => {
    if (err) return res.send({ msg: err.message });
      ...
      const sqlQuery = `SELECT cust_id FROM ${tableName} WHERE email=?`;
      connection.query(sqlQuery, req.params.email, (err, result) => {
        if (err) return res.send("Invalid Email");
        else if (result) {
          const row = JSON.parse(JSON.stringify(result));
          if (result.length === 0) {
            res.send({ msg: "NO_USER_FOUND" });
          } else {
            res.json({ msg: "success", cust_id: row[0].id });
          }
        }
      });
    }
...
const registerCustomer = async (req, res) => {
  mysqlPool.getConnection((err, connection) => {
    if (err) return res.send({ msg: err.message });
      ...
      console.log("register customer api activated");
      ...
      if (password) {
        bcrypt
          ...
          .then(async (hashedPassword) => {
            const sqlQuery = `INSERT INTO ${tableName} (name, email, password) VALUES(?,?,?)`;
            connection.query(
              sqlQuery,
              ...
              (err, result) => {
                if (err) return res.status(500).send({ msg: err.message });
                  ...
                  res.json({
                    msg: "success",
                    custid: Number(result.insertId.toString()),
                  });
                }
              }
            );
